...
@Model.register("preposition_role_bert")
class PrepSupersenseClassifierBert(Model): # Model inherits from torch.nn.Module and Registrable
  ...
  def __init__(
      ...
      self.num_classes = 55
      #self.num_classes2 = 60 #47 #38 #self.vocab.get_vocab_size("supersense2_labels")
  ...
  def forward(
      ...
      for i in range(prep_indices.shape[0]):
        ...
        labels = logits[i][current_idx] # the ith instance, the jth word (corresponds to the first word of the preposition)
      ...
      if supersense1_labels is not None:
        loss = self.loss(logits, supersense1_labels.long().view(-1))
        output_dict["loss"] = loss

      self.accuracy(logits, supersense1_labels)
  ...
  def decode(
      ...
      output_dict["supersense1_label"] = classes
  ...
  default_predictor = "preposition_role"
