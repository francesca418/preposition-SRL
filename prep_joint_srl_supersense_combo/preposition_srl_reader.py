...
@DatasetReader.register("preposition_srl_joint")
class SrlReader(DatasetReader):
  ...
  def _read(self, file_path: str):
    ...
    for (sentence, predicate_location, supersense, tags, filename) in srl_data:
      tokens = [Token(t) for t in sentence]
      prep_indicator = [1 if "p" in tag else 0 for tag in tags]
      yield self.text_to_instance(tokens, prep_indicator, supersense, tags, filename)
  ...
  def read_prep_srl(self, filename):
    ...
    for line in f.readlines():
      ...
      supersense = supersense1 + "." + supersense2
      ...
      for tag in tags:
          if "PREP" in tag:
              tag = tag.replace("PREP", supersense)
              #print(tag)
          new_tags.append(tag)
      assert len(new_tags) == len(sentence)
      print(str(new_tags))
      data.append((sentence, predicate_location, supersense, new_tags, filename))
    f.close()
    return data

  def text_to_instance(self, tokens: List[Token], prep_label: List[int], supersense: str, tags: List[str]=None, filename: str=None) -> Instance:
    '''
    ...
    if supersense:
      metadata_dict["supersense"] = supersense
