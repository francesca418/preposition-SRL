# pre-processing script to convert dependency parsed object and governor from streusle into text annotation json format
...
def convert():
    '''
    Load the Streusle dataset and convert into the format to text annotation JSON.
    ...
    Objects and Governors are labeled based on how they were annotated by the dependency parser.
    '''
    ...
    for sentence_dict in sentence_dict_list:
                ...
                for mw_dict in sentence_dict["smwes"].values():
                    try:
                        if token_num == mw_dict["toknums"][0]:
                            ...
                            if mw_dict["heuristic_relation"]["govlemma"] == "null":
                                gov_start = -1
                                gov_end = -1
                            else:
                                gov_start = mw_dict["heuristic_relation"]["gov"] - 1
                                gov_end = mw_dict["heuristic_relation"]["gov"]
                            if mw_dict["heuristic_relation"]["objlemma"] == "null":
                                obj_start = -1
                                obj_end = -1
                            else:
                                obj_start = mw_dict["heuristic_relation"]["obj"] - 1
                                obj_end = mw_dict["heuristic_relation"]["obj"]
                            prep_list.append({"preposition": mw_dict["lexlemma"], "start": token_counter, "end": endpoint, "governor": mw_dict["heuristic_relation"]["govlemma"], "gov_start":  gov_start, "gov_end": gov_end, "object": mw_dict["heuristic_relation"]["objlemma"], "obj_start": obj_start, "obj_end": obj_end})
                            num_preps += 1
                    except KeyError:
                        skipped += 1
                        #sys.stdout.write("\n" + "skipped" + str(skipped) + "\n")
                    ...
                    try:
                        if token_num == w_dict["toknums"][0]:
                            ...
                            if new_prep:
                                ...
                                if w_dict["heuristic_relation"]["govlemma"] == "null":
                                    gov_start = -1
                                    gov_end = -1
                                else:
                                    gov_start = w_dict["heuristic_relation"]["gov"] - 1
                                    gov_end = w_dict["heuristic_relation"]["gov"]
                                if w_dict["heuristic_relation"]["objlemma"] == "null":
                                    obj_start = -1
                                    obj_end = -1
                                else:
                                    obj_start = w_dict["heuristic_relation"]["obj"] - 1
                                    obj_end = w_dict["heuristic_relation"]["obj"]
                                prep_list.append({"preposition": w_dict["lexlemma"], "start": token_counter, "end": endpoint, "governor": w_dict["heuristic_relation"]["govlemma"], "gov_start":  gov_start, "gov_end":  gov_end, "object": w_dict["heuristic_relation"]["objlemma"], "obj_start": obj_start, "obj_end": obj_end})
                                num_preps += 1
                    except KeyError:
                        skipped += 1 
                        #sys.stdout.write("\n" + "skipped" + str(skipped) + "\n")
        ...
        for val in range(num_preps):
            ...
            obj_dict["label"] = "OBJ"
            obj_dict["score"] = 1.0
            obj_dict["start"] = current_preposition_dict["obj_start"]
            obj_dict["end"] = current_preposition_dict["obj_end"]
            if obj_dict["start"] != -1:
                data_dict["constituents"].append(obj_dict)
            gov_dict = {}
            gov_dict["label"] = "GOV"
            gov_dict["score"] = 1.0
            gov_dict["start"] = current_preposition_dict["gov_start"]
            gov_dict["end"] = current_preposition_dict["gov_end"]
            if gov_dict["start"] != -1:
                data_dict["constituents"].append(gov_dict)
