'''
This script converts Test Annotation JSON formatted preposition SRL data to formatted SRL data for reading into the model
...
def convert():
    ...
    streusle = []
    with open ("streusle.ud_train.govobj.json", "r") as f:
        streusle = json.load(f)

    with open("streusle-train.conll", "w") as f:
        f.write("")

    count = 0

    for fileName in os.listdir("annotated-ta/"):

        if fileName in files_to_ignore:
            continue

        current_dict = {}
        with open(os.path.join("annotated-ta", fileName), "r") as f:
        ...
        with open("streusle-train.conll", "a") as f:
            f.write(output_line + "\n")
